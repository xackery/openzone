...
uses SysUtils, Textures, {Global, q3timer,} qsortunit{, fmoddyn, fmodtypes, fmoderrors}, StrUtils;

Const
  Tesselation = 2;             // subdivision of bezier patches 2, 8 (3^n - 1)
  Gamma       = 2;
...
//    Cons.AddMsg('Effects = ' + IntToStr(numOfEffects));
  ...
  try
    ...
    begin
      ...
      if Faces[i].FaceType = FACE_PATCH then PrepareQuadricPatch(i)
      else
      begin
        point := SetVector(0, 0, 0);
        If Faces[i].numMeshVerts > 0 Then
        Begin
