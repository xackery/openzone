...
uses StdCtrls, sysutils, classes, GL,{OpenGL12,} Hashes;
...
splitLine:
    ...
    if (Length(line) > 0) and required then begin
      ...
      // Delphi StrToFloat wants leading zeroes for decimal numbers (i.e. no ".5")


      For I := 0 To Params.Count - 1 Do
      Begin
        If Copy(Params[I],1,1) = '.' Then Params[I] := '0' + Params[I];
        If Copy(Params[I],1,2) = '-.' Then Params[I] := '-0' + Copy(Params[I],2,Length(Params[I]));
      End; // For I
...
{          if params[1] = 'alphashadow' then
          ...
          try
          newItem.Surface := newItem.Surface + LookUp(params[1]);
          except
        ...
        if params[0] = 'cull' then begin
           newItem.Cull := LookUp(params[1]);
           continue;
        end;

        if params[0] = 'sort' then begin
          tmpInt := LookUp(params[1]);
          if tmpInt >   -1 then
        ...
        if params[0] = 'skyparms' then begin
          ...
          if (params[1] = '-') or IsNumeric(params[1]) or (params[1] = 'full') then begin // type = sphere
            newItem.SkyboxType := 1;
          end
          else begin                   // type = box
            ...
            newItem.SkyboxTextureNames[0] := ConvertPath(params[1]+'_rt');
            newItem.SkyboxTextureNames[1] := ConvertPath(params[1]+'_lf');
            newItem.SkyboxTextureNames[2] := ConvertPath(params[1]+'_ft');
            newItem.SkyboxTextureNames[3] := ConvertPath(params[1]+'_bk');
            newItem.SkyboxTextureNames[4] := ConvertPath(params[1]+'_up');
            newItem.SkyboxTextureNames[5] := ConvertPath(params[1]+'_dn');
            for i := 0 to 5 do
        ...
        if params[0] = 'deformvertexes' then begin
          ...
          newItem.DeformVertexes[newItem.numOfDeformVertexes-1].deformType := enDeformType(Lookup('def_'+params[1]));
            ...
            case deformType of
              ...
              DEFORMVERTEXES_WAVE :
                begin
                  for i := 0 to 5 do
                    if i = 1 then
                      Values[i] := Lookup(params[2+i])
                    else
                      Values[i] := StrToFloat(params[2+i]);
                end;
        ...
        if params[0] = 'fogparms' then begin
          ...
          newItem.Fog.Red := StrToFloat(params[2]);
          newItem.Fog.Green := StrToFloat(params[3]);
          newItem.Fog.Blue := StrToFloat(params[4]);
          newItem.Fog.Alpha := 1;
          newItem.Fog.Distance := StrToFloat(params[6]);
          continue;
        end;

        if params[0] = 'qer_editorimage' then begin
          newItem.qerImage := ConvertPath(params[1]);
          continue;
        end;
        ...
        if params[0] = 'depthfunc' then begin
          newItem.Layers[curLayer-1].DepthFunc := LookUp(params[1]);
        ...
        if params[0] = 'map' then begin
          ...
          newItem.Layers[curLayer-1].TextureNames[0] := ConvertPath(params[1]);
          continue;
        end;
        ...
        if params[0] = 'animmap' then begin
          newItem.Layers[curLayer-1].animFreq := StrToFloat(params[1]); // animation speed
          ...
          for i := 0 to params.Count-3 do begin
            newItem.Layers[curLayer-1].TextureNames[i] := ConvertPath(params[2+i]);
          end;
          continue;
        end;
        ...
        if params[0] = 'alphafunc' then begin
          newItem.Layers[curLayer-1].AlphaFunc := Lookup(params[1]);
        ...
        if params[0] = 'alphagen' then begin
          if params.Count = 5 then begin // must be a wave
            with newItem.Layers[curLayer-1].AlphaWave  do begin
              func := enWave(Lookup(params[2]));
              base := StrToFloat(params[3]);
              amp := StrToFloat(params[4]);
              phase := StrToFloat(params[5]);
              freq := StrToFloat(params[6]);
            end;
          end;
          continue;
        end;
        if params[0] = 'tcmod' then begin
          ...
          case tmpInt of
            1 : newItem.Layers[curLayer-1].tcmods[newItem.Layers[curLayer-1].numOftcmods-1].Values[0] := StrToFloat(params[2]); // rotate
            2,3 : begin // scale, scroll
                    for i := 0 to 1 do
                      newItem.Layers[curLayer-1].tcmods[newItem.Layers[curLayer-1].numOftcmods-1].Values[i] := StrToFloat(params[2+i]);
                  end;
            4 : begin // stretch
            ...
            6 : begin // turb
                  ...
                  newItem.Layers[curLayer-1].tcmods[newItem.Layers[curLayer-1].numOftcmods-1].Values[1] := StrToFloat(params[3]);
                  newItem.Layers[curLayer-1].tcmods[newItem.Layers[curLayer-1].numOftcmods-1].Values[2] := StrToFloat(params[4]);
                  newItem.Layers[curLayer-1].tcmods[newItem.Layers[curLayer-1].numOftcmods-1].Values[3] := StrToFloat(params[5]);
                end;
          end;
          continue;
        end;
        if params[0] = 'rgbgen' then begin
          ...
          newItem.Layers[curLayer-1].rgbGen[rgbIdx].rgbGType := enRGBGen(LookUp(params[1]));
